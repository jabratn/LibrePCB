...
QList<FileSystemRef> Library::searchForElements() noexcept {
  ...
  foreach (const QString& dirname,
           mFileSystem.getSubDirs(ElementType::getShortElementName())) {
    FileSystemRef elementFileSystem(mFileSystem, dirname);
    if (isValidElementDirectory<ElementType>(elementFileSystem)) {
      list.append(elementFileSystem);
    } else {
      qWarning() << "Directory is not a valid library element:"
                 << elementFileSystem.getPrettyPath();
    }
  }
  return list;
}
